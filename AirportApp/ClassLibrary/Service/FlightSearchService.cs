...
namespace AirportApp.Src.Service
{
    public class FlightSearchService : IFlightSearchService
    {
        ...
        {
            return await this.flightRepository.GetFlightByIdAsync(id);
        }

        public async Task<IEnumerable<Flight>> GetFlightsByRouteAsync(string location, string routeType, DateTime? date)
        {
            return await this.flightRepository.GetFlightsByRouteAsync(location, routeType, date);
        }

        public async Task<int> GetOccupiedSeatCountAsync(int flightId)
        {
            return await this.flightRepository.GetOccupiedSeatCountAsync(flightId);
        }
    }
}